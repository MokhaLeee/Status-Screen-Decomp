...
void DrawStatScreen(Proc* proc){
	...
	// Right Page
	u32 src = 0;
	CpuFastSet(&src,gpStatScreenPageBg0Map,0x1000140);
	CpuFastSet(&src,gpStatScreenPageBg0Map,0x1000140);
	new_gStatScreenDrawCallTable[gStatScreenBuffer.CurPage]();
	
	// Apply TSA
	BgMapCopyRect(gpStatScreenPageBg0Map,&gBg0MapBuffer[0x4C],0x12,0x12);
	BgMapCopyRect(gpStatScreenPageBg2Map,&gBg2MapBuffer[0x4C],0x12,0x12);
...
void MSS_DrawPage2_ItemPage(void){
	...
	if( (0x62!=cClassId) & (0x34!=cClassId) & (0!=unit->items[0]) ) // 0x62 and 0x34 are both Gorgon Egg; 
